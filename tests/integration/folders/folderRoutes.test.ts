...
async function contextCreate(): Promise<{
  ...
  const user = identityTestUserCreate("folder-user", { name: "folder-user", passwordIterations: 600_000 })
...
test("folder routes hide nonexistent and foreign folders behind the upstream invalid-folder response", async () => {
  const context = await contextCreate()
  const foreignUser = identityTestUserCreate("foreign-user", { name: "foreign-user", passwordIterations: 600_000 })
