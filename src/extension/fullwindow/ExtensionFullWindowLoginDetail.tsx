...
export function ExtensionFullWindowLoginDetail(p: ExtensionFullWindowLoginDetailProps) {
  return (
    <ExtensionCardWrapper class="p-4" aria-label={`Details of ${p.login.name}`}>
      <div class="flex items-start justify-between gap-2">
        <div class="min-w-0">
          <h2 class="truncate text-base font-semibold">{p.login.name}</h2>
          <p class="extension-muted-text truncate text-sm">{p.login.username ?? "No username"}</p>
          <p class="extension-muted-text truncate text-xs">{p.login.uri ?? "No URI"}</p>
        </div>
      ...
      <Show
        ...
        fallback={<p class="extension-muted-text text-sm">No copyable fields.</p>}
      >
