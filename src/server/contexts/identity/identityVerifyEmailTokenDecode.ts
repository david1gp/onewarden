...
type IdentityVerifyEmailClaims = {
  nbf: number
  exp: number
  iss: string
  sub: string
...
export async function identityVerifyEmailTokenDecode(
  token: string,
  issuer: string,
  publicKey: KeyInput | undefined,
  clock: Clock,
): Promise<Result<IdentityVerifyEmailClaims>> {
  const op = "identityVerifyEmailTokenDecode"
