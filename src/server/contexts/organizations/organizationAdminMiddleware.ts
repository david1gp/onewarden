...
export function organizationAdminMiddleware(
  ...
  return async (context, next) => {
    ...
    if (!organizationMembershipRoleCheck(guardResult.data.membership, "admin"))
      return apiErrorResponseCreate(
        authenticationGuardErrorCreate(
          "organizationAdminMiddleware",
          "You need to be Admin or Owner to call this endpoint",
        ),
      )
