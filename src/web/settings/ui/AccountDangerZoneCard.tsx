    ...
    <CardWrapper class="overflow-hidden rounded-xl border border-red-200 bg-white p-6 shadow-sm dark:border-red-900/60 dark:bg-slate-900">
      <div class="flex items-center gap-3 border-b border-red-100 pb-4 dark:border-red-950">
        ...
        <div>
          ...
          <p class="text-xs text-red-600 dark:text-red-400">Irreversible and destructive account actions</p>
        </div>
      </div>
      ...
      <div class="mt-6 space-y-6">
        <div>
          ...
          <div class="mt-4 flex items-center gap-3">
            <Button
              ...
              variant="outline"
              size="sm"
              class="text-xs text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/40"
        ...
        <Show when={state.isDeleteDialogOpen()}>
          <div class="rounded-xl border border-red-300 bg-red-50/80 p-5 dark:border-red-900/80 dark:bg-red-950/40">
            ...
            <div class="mt-4 space-y-3 max-w-md">
              <div>
                <Label for="del-confirm-text" class="block text-xs font-medium text-red-900 dark:text-red-200">
                ...
                <div class="mt-1">
                  <Input
                    ...
                    class="h-9 w-full rounded-md border-red-300 bg-white px-3 text-xs dark:border-red-800 dark:bg-slate-900"
                  />
                </div>
              </div>

              <div>
                <Label for="del-pwd" class="block text-xs font-medium text-red-900 dark:text-red-200">
                  Master Password
                </Label>
                <div class="mt-1">
                  <Input
              ...
              <div>
                <Label for="del-otp" class="block text-xs font-medium text-red-900 dark:text-red-200">
              ...
              <div class="pt-2 flex items-center gap-3">
                <Button
                  ...
                  variant="filled"
                  size="sm"
                  class="h-9 bg-red-600 hover:bg-red-700 text-white text-xs"
                  onClick={state.handleDeleteAccount}
                  disabled={state.isDeleting()}
                >
                  {state.isDeleting() ? "Deleting..." : "Permanently Delete Account"}
                </Button>
                <Button type="button" variant="ghost" size="sm" class="h-9 text-xs" onClick={state.closeDeleteDialog}>
                  Cancel
                </Button>
              </div>
            </div>
        ...
        <Show when={state.isRecoveryMode()}>
          ...
          >
            <h4 class="font-semibold text-xs text-slate-900 dark:text-slate-100">Request Deletion Email</h4>
            <p class="text-xs text-slate-600 dark:text-slate-400">
              If you lost access to your master password, request an email with a deletion confirmation link.
            </p>
            <div>
              <Label for="rec-del-email" class="block text-xs font-medium text-slate-700 dark:text-slate-300">
                Account Email
              </Label>
              <div class="mt-1">
                <Input
            ...
            <div class="pt-1">
              <Button type="submit" variant="filled" size="sm" class="h-9 text-xs" disabled={state.isSendingRecovery()}>
