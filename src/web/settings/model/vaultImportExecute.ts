...
import { bitwardenAccountEncryptedJsonEnvelopeDecrypt } from "./bitwardenAccountEncryptedJsonEnvelopeDecrypt.js"
...
export async function vaultImportExecute(
  ...
  let accountEncryptedEnvelope: unknown | undefined
  if (options.format === "json") {
    ...
    if (rawPayloadRecord !== null && "encrypted" in rawPayloadRecord && rawPayloadRecord.encrypted === true) {
      if (rawPayloadRecord.passwordProtected === true) {
        ...
        accountEncryptedEnvelope = rawPayload
      }
    } else {
  ...
  let parsedFoldersForCleanup: unknown = null
  let parsedItemsForCleanup: unknown = null
  let csvRecordsForCleanup: unknown = null
  let csvFolderNamesForCleanup: Set<string> | null = null
  let folderIndexMapForCleanup: Map<string, number> | null = null
  try {
    if (accountEncryptedEnvelope !== undefined) {
      const decryptedPayloadResult = await bitwardenAccountEncryptedJsonEnvelopeDecrypt(
        accountEncryptedEnvelope,
        userKey,
      )
      if (!decryptedPayloadResult.success) {
        return resultErrorCreate(
          op,
          `Account-restricted Bitwarden encrypted JSON import failed: ${decryptedPayloadResult.errorMessage}`,
          { code: decryptedPayloadResult.code, statusCode: decryptedPayloadResult.statusCode },
        )
      }
      jsonPayload = decryptedPayloadResult.data
    }
    ...
    bitwardenAccountEncryptedJsonSensitiveValueClear(jsonPayload)
    bitwardenAccountEncryptedJsonSensitiveValueClear(parsedFoldersForCleanup)
    bitwardenAccountEncryptedJsonSensitiveValueClear(parsedItemsForCleanup)
    bitwardenAccountEncryptedJsonSensitiveValueClear(csvRecordsForCleanup)
    csvFolderNamesForCleanup?.clear()
    folderIndexMapForCleanup?.clear()
  }
}
