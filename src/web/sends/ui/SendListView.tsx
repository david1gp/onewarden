    ...
    <div class="mx-auto max-w-5xl px-4 py-8">
      ...
      <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <Button type="button" variant="filled" size="sm" class="text-xs" onClick={state.openCreate}>
                ...
                {(item) => (
                  <div class="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-start gap-3">
                      ...
                      <div class="min-w-0">
                        <div class="flex items-center gap-2">
                          <span class="font-semibold text-xs text-slate-900 dark:text-slate-100">{item.name}</span>
                          <Show when={item.disabled}>
                            <Badge
                              variant="subtle"
                              class="bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 text-[10px]"
                            >
                              Disabled
                            </Badge>
                          </Show>
                          <Show when={item.authType === 1}>
                            <Badge
                              variant="subtle"
                              class="bg-indigo-100 text-indigo-800 dark:bg-indigo-950 dark:text-indigo-300 text-[10px]"
                            >
                              Password Protected
                            </Badge>
                          </Show>
                        ...
                        <Show when={item.notes}>
                          <p class="mt-1 text-[11px] text-slate-400 dark:text-slate-500 italic">Note: {item.notes}</p>
                        </Show>
                      </div>
                    </div>
