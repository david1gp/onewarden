...
export function VaultEmptyState(props: VaultEmptyStateProps = {}): JSX.Element {
  ...
  return (
    <div class="flex h-full w-full flex-col items-center justify-center p-8 text-center">
      ...
      <p class="mt-1 max-w-sm text-xs text-slate-600 leading-relaxed dark:text-slate-400">{state.description()}</p>
      <Show when={state.actionLabel()}>
        <Button variant="filledBlue" size="sm" class="mt-4 text-xs font-semibold" onClick={state.handleAction}>
          {state.actionLabel()}
        </Button>
      </Show>
    </div>
